...
test("候補が4種そろっても最大3件で復習を先頭にし未回答ミッションを除外する", () => {
  ...
  const state: ProgressState = {
    ...createProgressState([
      createAnswer({ questionId: "part5-001", part: "part5", correct: true, tags: ["grammar"] }),
      createAnswer({ questionId: "part6-001-1", part: "part6", correct: false, tags: ["context"] }),
      createAnswer({ questionId: "part7-001-1", part: "part7", correct: true, tags: ["grammar"] }),
    ]),
    srs: {
      "part5-001": {
        questionId: "part5-001",
...
test("未回答ミッションの遷移先が苦手Partミッションと重複する場合は追加しない", () => {
  // part5 を1問だけ不正解にすると、苦手Part・未回答Partがともに part5 となり href が重複する。
  const state = createProgressState([
    createAnswer({
      questionId: "part5-001",
      part: "part5",
  ...
  const partFiveHrefs = missions.filter(
    (mission) => mission.href === "/practice?mode=part&part=part5",
  );
  assert.equal(partFiveHrefs.length, 1);
  assert.equal(missions.some((mission) => mission.kind === "unanswered"), false);
});
