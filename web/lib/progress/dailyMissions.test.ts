...
test("未回答ミッションは未回答優先導線で既回答より未回答を先に出す", () => {
  ...
  const expectedUnansweredQuestion = questionsByPart[part].find(
    (item) =>
      !state.answers.some((answer) => answer.questionId === item.questionId),
  );
  ...
  assert.equal(
    unansweredMission.href,
    `/practice?mode=part&part=${part}&unanswered=1`,
  );
