...
test("進捗なし状態では全体到達率を0件として集計する", () => {
  ...
  assert.equal(summary.total, 364);
  assert.equal(summary.answered, 0);
  assert.equal(summary.unanswered, 364);
...
test("問題データ追加時は総数と未回答数が渡された問題参照に合わせて増える", () => {
  ...
  assert.equal(summary.total, 365);
  assert.equal(summary.unanswered, 365);
  assert.equal(summary.parts[0].total, 231);
});
