...
現状は **Phase 1 スケルトン段階**: ルート・型・設定は揃っているが、各ページは見出しだけのスタブ、`web/components/` と `web/lib/` は空（`.gitkeep` のみ）、`web/data/*.json` は空配列。**実装の設計指針は `docs/` を正とする**（後述）。
...
- **データの流れ（2 系統）**:
  ...
  - 進捗データ = ブラウザの `localStorage`（キー `toeicReadingProgress:v2`、型は `ProgressState`）。アプリ起動時に読み込み、セッション完了時に保存する。旧 `toeicReadingProgress:v1` は初回読み込み時に v2 へ移行する。
...
- ルート: `/`（ホーム）, `/practice`（演習）, `/review`（復習）, `/progress`（進捗）, `/settings`（設定）。Phase 1 は計 12 画面で、画面遷移・状態更新・例外処理は `docs/WIREFLOW.md` が正。
- `components/{practice,progress,shared}/` と `lib/{question-bank,progress,srs,storage,validation}/` は **意図された配置（現在は空）**。実装はこの構成に沿って追加する。
...
- `docs/WIREFLOW.md` — 12 画面の遷移図・画面別イベント・状態更新タイミング・エラー復旧導線
