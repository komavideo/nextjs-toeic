...
現状は **0.1.0 公開候補段階**: 主要ルート、演習・復習・進捗・設定 UI、進捗保存、SRS、問題バンク、検証スクリプト、304問の静的問題データが実装済み。**実装の設計指針は `docs/` を正とする**（後述）。
...
- ルート: `/`（ホーム）, `/practice`（演習）, `/review`（復習）, `/progress`（進捗）, `/settings`（設定）。Phase 1 は拡張画面を含む計 13 画面で、画面遷移・状態更新・例外処理は `docs/WIREFLOW.md` が正。
- `components/{practice,progress,shared}/` と `lib/{question-bank,progress,srs,storage,validation}/` が主な配置先。追加実装はこの構成に沿って行う。
