...
- **データの流れ（2 系統）**:
  ...
  - 進捗データ = ブラウザの `localStorage`（キー `toeicReadingProgress:v3`、型は `ProgressState`）。アプリ起動時に読み込み、セッション完了時に保存する。旧 `toeicReadingProgress:v1` / `v2` は初回読み込み時に v3 へ移行する（移行元キーは削除するため、本バージョン以降は旧版へのロールバックで進捗を引き継げない）。`ProgressState` は獲得バッジ（`unlockedBadges`、バッジ ID → 解除日時 ISO）を含む。
