...
flowchart TD
  ...
  Due -->|No| Wait
  Due -->|Yes| Start
  Bookmarked -->|Yes| BookmarkList
  Bookmarked -->|No| Due
  BookmarkList --> BookmarkStart
  BookmarkStart --> Practice
  Start --> Practice
  Practice --> Grade
  Grade --> Correct
  Correct -->|Yes| Extend
...
復習予定は期限切れ / 今日 / 明日以降の3グループに分類して一覧表示する。復習開始の対象は期限到来（期限切れ・今日）のみで、明日以降だけの場合は一覧を表示しつつ復習開始ボタンは出さない。ブックマーク済み問題がある場合は、期限到来とは別にブックマーク一覧とブックマーク復習開始ボタンを表示する。復習予定もブックマークもない場合のみ空状態を表示する。
