...
2. `http://localhost:4173/`、`/practice/`、`/review/`、`/progress/`、`/settings/` に直接アクセスし、各ルートが HTTP 200 で表示されることを確認する。
3. 静的配信上のホームで「今日の学習ミッション」から `/practice/?mode=review`、`/practice/?mode=part&part=...&tag=...`、`/practice/?mode=part&part=...&unanswered=1` が空白画面や console error なしで開始できることを確認する。
