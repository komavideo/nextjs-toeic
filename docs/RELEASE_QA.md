...
1. 主要フローとして、初回ホームから5問クイックを開始し、回答、採点、解説、結果、ホーム復帰、進捗反映まで確認する。
2. 復習フローとして、復習対象なしの表示と、期限到来データ投入後の復習一覧、復習開始、回答、SRS 更新、結果表示を確認する。
3. レスポンシブ確認として、360px 前後のスマートフォン幅、768px 前後のタブレット幅、1120px 以上のデスクトップ幅で、ホーム、演習、復習、進捗、設定の主要画面に横スクロール、本文とナビゲーションの重なり、固定アクションバーの干渉、空白画面、Next.js エラー overlay がないことを確認する。
4. A11y 確認として、主要操作のキーボード到達、フォーカスリング、モーダルの `role="dialog"` / `aria-modal` / フォーカストラップ / Escape 終了 / フォーカス復帰、必要に応じた Lighthouse Accessibility スコアを確認する。
5. 権利表記として、初回ホーム、通常ホーム、設定画面で ETS 非提携とオリジナル問題方針が表示され、TOEIC をアプリ名、ロゴ、主ブランドとして使っていないことを確認する。
...
3. 各主要ルートでブラウザリロードしても 404、空白画面、Next.js エラー overlay が出ないことを確認する。
4. ブラウザ console の warning/error が 0 件であること、ローカル静的アセット以外への外部リクエストが発生していないことを確認する。
5. 実ホスティング URL が確定している場合は、同じ観点を実 URL でも確認し、ローカル確認とは別行で記録する。
