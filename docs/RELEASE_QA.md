...
2. 通常ホームの「今日の学習ミッション」で、復習、苦手 Part、苦手タグ、未回答優先（`unanswered=1`）の各導線が該当する演習へ遷移し、未回答優先では既回答 `questionId` が先頭キューから除外されることを確認する。
...
- ホームが空白化する、ミッション CTA が遷移しない、未回答優先ミッションで既回答問題が再出題される、または静的配信で console error が再現する場合はリリースを止めて直前版へ戻す。
