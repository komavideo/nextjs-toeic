...
既存の `toeicReadingProgress:v1` / `toeicReadingProgress:v2` は、初回読み込み時に `version: 3` 形式へ移行する。自動移行では移行元キーを削除せず、v3 保存時に `toeicReadingProgress:v2` の互換スナップショットも維持する。明示的なデータリセット時のみ v1 / v2 / v3 をまとめて削除する。
...
type AnswerResult = {
  ...
  elapsedMs: number;
  tags: string[];
  // 新規回答では保存する。旧データ互換のため読み込み時は任意項目として扱う。
  sessionId?: string;
};
